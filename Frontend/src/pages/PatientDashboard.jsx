...
const PatientDashboard = () => {
  ...
  const stats = [
    ...
    {
      label: "Completed",
      value: reports.filter((r) => r.status === "PROCESSED").length,
      icon: CheckCircle2,
      color: "text-medic-accent",
      bg: "bg-medic-light/50",
    },
  ];
  ...
  const getStatusStyle = (status) => {
    switch (status) {
      case "PROCESSED":
        return "bg-medic-light text-medic-dark border-medic-dark/10";
      case "PENDING":
    ...
    <div className="max-w-7xl mx-auto px-6 py-10 space-y-10">
      ...
      >
        <div className="relative z-10 max-w-2xl">
          ...
          <p className="text-medic-light/70 text-base sm:text-lg leading-relaxed">
            Track and review your medical reports below. Our AI is ready to
            explain your latest results.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {stats.map((stat, i) => (
          ...
          >
            <div
              className={`w-14 h-14 ${stat.bg} ${stat.color} rounded-2xl flex items-center justify-center flex-shrink-0`}
            >
      ...
      <section className="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-8 border-b border-gray-50 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto">
            ...
            <Link
              to="/check-reports"
              className="bg-medic-dark text-white p-2.5 rounded-xl hover:bg-medic-primary transition-all"
            >
          ...
          {loading ? (
              ...
              <tbody className="divide-y divide-gray-50">
                {reports.map((report) => (
                  ...
                  >
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-bold text-gray-900">
                          {report.file?.split("/").pop() || "Medical Report"}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-8 py-5">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-bold border ${getStatusStyle(report.status)}`}
                      >
                        {report.status}
                      </span>
                    </td>
                    <td className="px-8 py-5 text-right">
                      <div className="flex flex-col items-end gap-2">
                        ...
                        {report.doctor_comment && (
                          <div className="mt-2 text-left bg-medic-light/20 p-3 rounded-xl border border-medic-dark/10 max-w-xs transition-all hover:bg-medic-light/30">
                            <div className="flex items-center gap-1.5 text-medic-dark mb-1">
            ...
            <div className="text-center py-20 px-8">
              ...
              <p className="text-gray-500 max-w-xs mx-auto mb-8">
                You haven't uploaded any medical reports yet. Start by adding
                your first report.
              </p>
              <Link
                to="/check-reports"
                className="inline-flex items-center gap-2 px-8 py-3 bg-medic-dark text-white rounded-xl font-bold hover:bg-medic-primary transition-all"
              >
