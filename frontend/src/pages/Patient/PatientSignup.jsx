...
function PatientSignup({ isEmbedded = false }) {
  ...
  useEffect(() => {
    // Load Google Identity Services script using unified service
    loadGoogleScript()
      .then(() => {
      ...
      if (window.google?.accounts?.id) {
        window.google.accounts.id.prompt((notification) => {
          if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
            // If prompt is not shown, we could fallback to the button or a custom popup
            console.log('One Tap prompt suppressed, user can use the Google button');
            // Check if we have a button container to render into
  ...
  const handleGoogleResponse = async (response) => {
    try {
      ...
      if (!userData) throw new Error('Failed to decode credential');

      console.log('Google Signup Success:', userData);

      // In a real app, you would send this to your backend
      // and wait for a response before navigating.
      // For now, mirroring the existing (placeholder) logic:
      navigate('/patient/login', {
        state: {
      ...
      <div className="flex gap-3 justify-center">
        {!isIOS() && (
          <div id="google-signup-btn" className="w-full">
            <button
              ...
              className="flex items-center justify-center gap-2 py-3 border border-slate-200 rounded-lg hover:bg-slate-50 transition w-full shadow-sm font-semibold"
        ...
        {isIOS() && (
          ...
          >
            ...
            <span className="font-semibold text-slate-700">Continue with Apple</span>
          </button>
        )}
      </div>
    </form>
