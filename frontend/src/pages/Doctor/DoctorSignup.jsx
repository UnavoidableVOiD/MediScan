...
function DoctorSignup({ isEmbedded = false }) {
  ...
  useEffect(() => {
    loadGoogleScript()
      .then(() => {
        console.log('Google Script loaded (Doctor Signup)');
      })
      .catch(console.error);
  }, []);
  ...
  const handleGoogleSignup = () => {
    try {
      ...
      if (window.google?.accounts?.id) {
        window.google.accounts.id.prompt((notification) => {
          if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
            console.log('Doctor One Tap suppressed');
            const googleBtn = document.getElementById('doctor-google-signup-btn');
            if (googleBtn) {
      ...
      alert('Google signup is not available.');
    }
  };
  ...
  const handleGoogleResponse = async (response) => {
    try {
      ...
      console.log('Doctor Google Signup Success:', userData);

      // Mirroring existing signup logic:
      navigate('/doctor/dashboard', { state: { welcomeMessage: `Welcome to Mediscan, Dr. ${userData.name.split(' ')[0]}!` } });
    } catch (error) {
      console.error('Doctor Google signup error:', error);
  ...
  const formContent = (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-3">
        {!isIOS() && (
          <div id="doctor-google-signup-btn" className="w-full">
            <button
              type="button"
              onClick={handleGoogleSignup}
              className="flex items-center justify-center gap-3 w-full py-3 border border-slate-200 rounded-lg hover:bg-slate-50 transition shadow-sm font-semibold text-slate-700 text-sm"
