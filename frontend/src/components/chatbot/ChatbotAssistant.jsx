...
const ChatbotAssistant = () => {
  ...
  const [messages, setMessages] = useState([
    {
      role: "bot",
      content:
        "Hello! I am your Mediscan Assistant. How can I help you understand your reports today?",
    },
  ]);
  ...
  const handleSend = async (e) => {
    ...
    setMessages((prev) => [
      ...prev,
      { role: "bot", content: "", id: botMsgId },
    ]);

    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || "http://localhost:8000/api"}/chatbot/chat/`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            patient_context: currentReport
              ? `User is discussing Report #${currentReport.id}`
            ...
            <div className="flex-grow overflow-y-auto p-6 space-y-6 bg-neutral-background/30">
              {messages.map((msg, i) => (
                ...
                >
                  ...
                  >
                    <div
                      className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${msg.role === "user" ? "bg-medic-dark text-white" : "bg-medic-light text-medic-dark"}`}
                    >
                      {msg.role === "user" ? (
                    ...
                    <div
                      className={`p-4 rounded-2xl text-sm leading-relaxed shadow-sm ${msg.role === "user" ? "bg-medic-dark text-white rounded-tr-none" : "bg-white text-gray-700 rounded-tl-none border border-gray-100"}`}
                    >
                      {msg.content}
                    </div>
                  </div>
            ...
            <div className="p-6 border-t border-gray-100 bg-white space-y-4">
              <div className="flex gap-2 p-3 bg-amber-50 rounded-xl border border-amber-100">
                ...
                <p className="text-[10px] text-amber-700 leading-tight">
                  This assistant provides informational guidance only and is not
                  a medical diagnosis.
                </p>
              </div>
